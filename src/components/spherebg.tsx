...
const SphereBg = () => {
  ...
  useEffect(() => {
    ...
    const keyLight = new DirectionalLight(0xfff5e0, 3.5);
    keyLight.position.set(6, 8, 6);
    keyLight.castShadow = true;
    scene.add(keyLight);

    const fillLight = new DirectionalLight(0xe0f0ff, 1.5);
    ...
    const spheres: Sphere[] = sphereConfigs.map((cfg) => {
      const geo = new SphereGeometry(cfg.r, 64, 64);
      ...
      for (let i = 0; i < spheres.length; i++) {
        for (let j = i + 1; j < spheres.length; j++) {
          ...
          if (dist < minDist && dist > 0.001) {
            ...
            // ✅ normalize() mutates _diff in place — safe to reuse for reflect
    ...
    // ─── Mouse parallax ─────────────────────────────────────────
    const targetCameraOffset  = new Vector2(0, 0);
    const currentCameraOffset = new Vector2(0, 0);

    // ✅ mousemove handler reads no layout properties — no reflow risk
    ...
    // ─── Resize ─────────────────────────────────────────────────
    // ✅ ResizeObserver is the correct API here — it fires AFTER layout is
    // committed by the browser, so reading contentRect causes zero reflow.
    // The old window "resize" + mount.clientWidth pattern forced a reflow
    // because clientWidth is read after the browser invalidates layout.
    const resizeObserver = new ResizeObserver((entries) => {
      // contentRect is provided by the browser — no property read, no reflow
    ...
    // ✅ Prevents the animation loop from running in the background,
    // which contributed to the continuous Framer Motion-like long tasks.
    let isVisible = true;
    ...
    const animate = () => {
      ...
      spheres.forEach(({ mesh, velocity, rotationSpeed }) => {
        ...
        // ✅ Cached `axes` array — avoids re-creating ["x","y","z"] every frame
      ...
      // ✅ renderer.render() writes to the canvas (GPU) — does NOT read
      // any layout properties, so no reflow is triggered here.
